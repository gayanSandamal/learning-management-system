            ...
            <div class="form">
              ...
              <div v-if="user.roleId === 1024" class="row">
                <div class="col-12 mb-3">
                  ...
                  <treeselect v-model="selectedCategoryIds" :multiple="true" :options="mappedCats" :flat="true"/>
                    ...
                    <tbody>
                      <tr v-for="selectedClass in selectedClasses" :key="selectedClass.id">
                        ...
                        <td>
                          ...
                          <textarea v-else class="form-control" rows="1" v-model="selectedClass.src" @input="setSrc(selectedClass.id, selectedClass.src)" placeholder="Paste demo video embed url"></textarea>
...
export default {
  ...
  methods: {
          ...
          if (this.selectedCategories.length > 0) {
            this.selectedCategories.map((p, i) => {
              if (p.category_id === o.id) {
                ...
                this.$set(o, 'assigned_item_id', p.id)
              }
    ...
    setSrc(class_id, passSrc) {
      setTimeout(() => {
        debugger
        let reference = document.querySelector(`#reference-${class_id}`)
        reference = reference.children[0]
        let src
        let id
        if (reference) {
          const iframe = reference
          src = iframe.getAttribute('src')
        } else {
          src = passSrc
          src = passSrc.replace('https://vimeo.com/', '')
          src = src.split("/")
          src = src[0]
        }
        if (src.includes('?')) {
